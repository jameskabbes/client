...
class Package(parent_class.ParentClass):
    ...
    _config_dict = {}
    _config_json_Path = params._Dir.join_Path(path='Package.json')

    def __init__(self, _Dir, dict={}, root=None):
        parent_class.ParentClass.__init__(self)

        if root == None:
            root = kabbes_client.root

        valid_root = isinstance(root, Root)

        # Load baseline
        self.cfg = kabbes_config.Node('TEMP', dict=Package._config_dict)
        self.cfg.merge(kabbes_config.Config(
            dict={'_Dir': _Dir, '_self': self}))

        # Load Package.json
        cfg_json = kabbes_config.Config(
            dict=Package._config_json_Path.read_json_to_dict())
        self.cfg.merge(cfg_json)

        # Load _Dir/CONFIG.json
        if self.cfg['package.config.Path'].exists():
            self.cfg.merge(kabbes_config.Config(
                dict=self.cfg['package.config.Path'].read_json_to_dict()))
